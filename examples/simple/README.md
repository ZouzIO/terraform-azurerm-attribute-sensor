...
| <a name="input_existing_export"></a> [existing\_export](#input\_existing\_export) | (*Optional*) Passed through to the module's `existing_export` input. | <pre>object({<br>    storage_container   = string<br>    storage_dir         = string<br>    storage_account_id  = string<br>    storage_export_type = string<br>  })</pre> | `null` | no |
