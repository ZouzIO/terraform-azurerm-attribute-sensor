...
## Using existing Cost Management Exports
If one or more Cost Management Exports already exist (created outside this module), set `create_costs_export = false` and pass their details via the `existing_exports` list. For every entry the module then:
- grants the managed identity `Storage Blob Data Reader` on the export's storage account (`storage_account_id`) — deduplicated, so multiple exports in the same account produce a single role assignment, and
- forwards `storage_container`, `storage_dir`, the storage account's blob endpoint (constructed from the account name in `storage_account_id`, assuming the Azure public cloud `*.blob.core.windows.net` suffix) and `storage_export_type` as an entry in the registration request's `storage_info` list.

When `existing_exports` is set the registration carries **only** the `storage_info` list — the legacy singular `storage_container` / `storage_dir` / `storage_account_url` / `storage_export_type` fields are not sent (they are still used for a module-created export).

An export's storage account may live in a different subscription than the provider's; the module never reads it, it only constructs the endpoint and assigns the role at its scope (so the deploying principal needs `Microsoft.Authorization/roleAssignments/write` on that storage account).

Each entry's `storage_export_type` must be either `csv` or `parquet-snappy`. `existing_exports` is mutually exclusive with `create_costs_export`.
...
```
Only each export's storage account resource ID is required — the module constructs the blob endpoint URL from the account name in it, so no separate URL input is needed.
...
| [azurerm_role_assignment.existing_exports](https://registry.terraform.io/providers/hashicorp/azurerm/latest/docs/resources/role_assignment) | resource |
...
| <a name="output_storage_info"></a> [storage\_info](#output\_storage\_info) | The list of existing Cost Management Exports forwarded as the registration's `storage_info`. Each entry carries `storage_container`, `storage_dir`, the derived `storage_account_url` and `storage_export_type`. Null when no `existing_exports` is provided. |
